...
const ContentRow: React.FC<{ title: string; movies: Movie[]; onCardClick: (movie: Movie) => void; zIndex?: number }> = ({ title, movies, onCardClick, zIndex }) => {
    ...
    return (
        <div className="my-6 md:my-8" style={{ zIndex }}>
            ...
            <div className="overflow-x-auto no-scrollbar py-32 -my-32"><div className="flex flex-nowrap gap-x-2">{movies.map(movie => <PosterCard key={movie.id} movie={movie} onCardClick={onCardClick} />)}</div></div>
        </div>
    );
};
